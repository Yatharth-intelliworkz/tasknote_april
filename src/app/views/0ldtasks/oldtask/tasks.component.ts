...
export class TasksComponent {
...

    {
      ...
      cellRenderer: (params: any) => {
        ...
        chatImg.addEventListener('click', () => {
          this.commenttask(params.data.pinTaskData.task_id);
        });
  ...
  public todayColumnDefs: ColDef[] = [
...

    {
      ...
      cellRenderer: (params: any) => {
        ...
        // Add click event for chat icon
        chatImg.addEventListener('click', () => {
          this.commenttask(params.data.todayTaskData.task_id);
        });
  ...
  public overDueColumnDefs: ColDef[] = [
    ...
    // Add click event for timer using arrow function to preserve `this`
      ...
      wrapper.appendChild(timerImg);
        ...
        // Add click event for chat icon
        chatImg.addEventListener('click', () => {
          this.commenttask(params.data.overDueTaskData.task_id);
        });
  ...
  public upComingColumnDefs: ColDef[] = [
...

    {
      ...
      cellRenderer: (params: any) => {
        ...
        // Add click event for chat icon
        chatImg.addEventListener('click', () => {
          this.commenttask(params.data.upcomingTaskData.task_id);
        });
  ...
  public completedColumnDefs: ColDef[] = [
...

    {
      ...
      cellRenderer: (params: any) => {
        ...
        // Add click event for chat icon
        chatImg.addEventListener('click', () => {
          this.commenttask(params.data.completedTaskData.task_id);
        });
  ...
  private lastOverdueCount: number = 0;
  ...
  ngOnInit() {
    if (localStorage.getItem('userid') !== null) {
      ...
      channel3.bind('push-notification', (data: any) => {
        console.log('Push notification received:', data, 'userId:', userId, 'data.message.userId:', data.message?.userId);
        
        // Check if notification is for current user
        if (userId === data.message?.userId || userId === parseInt(data.message?.userId || '0', 10)) {
          // Show desktop notification
          this.notificationService.pushNotify(data.message);
          
          // Also trigger a page reload to fetch latest tasks
          // setTimeout(() => {
  ...
  gettaskList() {
    ...
    if (token) {
      const headers = this.commonService.getAuthHeaders(token);
      const body = this.commonService.getTaskRequestPayload(true);
      ...
      this.http.post(`${this.apiUrl}myTaskList`, body, { headers }).subscribe(
        (response: any) => {
          ...
          const overdueCount = this.overDueListRowData?.length || 0;
          if (overdueCount > 0 && overdueCount !== this.lastOverdueCount) {
            this.notificationService.pushNotify({
              title: 'Overdue Tasks',
              body: `You have ${overdueCount} overdue task${overdueCount > 1 ? 's' : ''}.`,
              icon: `${window.location.origin}/assets/img/dashboard/notification.svg`,
              url: `${window.location.origin}/tasks`,
            });
            this.lastOverdueCount = overdueCount;
          }

          this.evaluatePendingTaskNotifications([
            ...(this.pinListRowData || []),
            ...(this.todayListRowData || []),
            ...(this.overDueListRowData || []),
            ...(this.upcomingListRowData || []),
          ]);
  ...
  private getTaskIdentifier(task: any): string {
    return task?.task_id?.toString?.() || task?.id?.toString?.() || 'unknown';
  }

  private evaluatePendingTaskNotifications(tasks: any[]) {
    const now = new Date();
    ...
    tasks.forEach((task: any) => {
      ...
      const isPending = task?.completed == 0 || task?.completed === false || task?.status !== 'Closed';

      if (!taskId || !dueDate || !isPending) {
        return;
      }

      const sameDayKey = `${taskId}:sameDay`;
      const duePlus5Hours = addHours(dueDate, 5);
      const after5hKey = `${taskId}:after5h`;

      if (isSameDay(dueDate, now) && !this.pendingTaskNotificationsShown.has(sameDayKey)) {
        notifications.push({
          title: 'Pending Task Due Today',
          body: `${task.title || 'A task'} is due today and still pending.`,
          icon: `${window.location.origin}/assets/img/dashboard/notification.svg`,
          url: `${window.location.origin}/tasks`,
        });
        this.pendingTaskNotificationsShown.add(sameDayKey);
      }

      if (now >= duePlus5Hours && !this.pendingTaskNotificationsShown.has(after5hKey)) {
        notifications.push({
          title: 'Pending Task Reminder',
          body: `${task.title || 'A task'} is still pending 5 hours after its due time.`,
          icon: `${window.location.origin}/assets/img/dashboard/notification.svg`,
          url: `${window.location.origin}/tasks`,
        });
        this.pendingTaskNotificationsShown.add(after5hKey);
      }
    });

    if (notifications.length > 0) {
      this.notificationService.pushNotify(notifications);
    }
  }
