...
export class ManageServiceComponent {
  ...
  getservicelist() {
    ...
    if (token) {
      ...
      const headers = new HttpHeaders()
        ...
        .set('Accept', 'application/json');
      this.http
        .get(`${this.apiUrl}serviceList?companyID=` + this.companyId, {
          headers,
        })
        .subscribe(
          (noteslistData: any) => {
  ...
  servicedelete(id: any) {
    ...
    if (token) {
      ...
      this.http
        .post(`${this.apiUrl}serviceDelete`,
          serviceID,
          { headers }
        )
        .subscribe(
  ...
  serviceedit(id: number) {
    ...
    if (token) {
      ...
      this.http.get(`${this.apiUrl}serviceGet?serviceID=` + id, { headers }).subscribe(
        (editnoteslistData: any) => {
  ...
  updateservice(): void {
    ...
    this.http.post(`${this.apiUrl}serviceEdit`, this.editForm.value, { headers }).subscribe(
      (response: any) => {
        if (response.status === true) {
