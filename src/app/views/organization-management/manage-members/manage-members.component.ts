...
export class ManageMembersComponent {
  ...
  public columnDefs: ColDef[] = [
    ...
    { field: 'phone_no', headerName: 'Mobile Number' },
    { field: 'is_active', headerName: 'Status' },
    { field: 'assignRole', headerName: 'Roles' },
    {
      cellRenderer: (params: any) => {
        ...
        if (editPermission) {
            ...
            editIcon.setAttribute('data-bs-target', '#editMemberModal');
    
            // Add click event listener for edit action
            editIcon.addEventListener('click', () => {
                this.EditMember(taskId);
            });
        ...
        if (deletePermission) {
            ...
            deleteIcon.setAttribute('data-bs-target', '#deletememberModal');
    
            // Add click event listener for delete action
            deleteIcon.addEventListener('click', () => {
                this.deletememberopendialogue(taskId);
            });
  ...
  usersPermission() {
    ...
    if (
      this.userpermission.add === 0 &&
      this.userpermission.edit === 0 &&
  ...
  addmember(information: any, clearaddform: any) {
    ...
    if (token) {
      ...
      this.http
        .post(`${this.apiUrl}comapnyMemberAdd`, formData, { headers })
        .subscribe(
          (response: any) => {
  ...
  DeleteMember(id: number) {
    ...
    if (token) {
      ...
      this.http
        .post(`${this.apiUrl}comapnyMemberDelete`, memberId, { headers })
        .subscribe(
          (response: any) => {
            if (response.status === true) {
              const elementToClick =
                this.elementRef.nativeElement.querySelector(
                  '.popup_close_btn_delete_member'
                );
              if (elementToClick) {
                elementToClick.click();
              }
  ...
  LoadProjectList() {
    ...
    if (token) {
      ...
      this.http
        .get(`${this.apiUrl}taskProjectList?companyID=${this.companyId}`, {
          headers,
        })
        .subscribe(
          (projectlistData: any) => {
  ...
  LoadMemberList() {
    ...
    if (token) {
      ...
      this.http
        ...
        .subscribe(
          (teamlistData: any) => {
            this.team = teamlistData.data.map(
  ...
  addTeam(): void {
    ...
    if (token) {
      ...
      const membersIds = this.submitForm.value.members_id
      ...
      this.http.post(`${this.apiUrl}teamAdd`, payload, { headers }).subscribe(
        (response: any) => {
          if (response.status === true) {
            const elementToClick = this.elementRef.nativeElement.querySelector(
              '.popup_close_btn_team_add'
            );
            if (elementToClick) {
              elementToClick.click();
            }
  ...
  EditMember(id: any): void {
    ...
    if (token) {
      ...
      const headers = new HttpHeaders()
        ...
        .set('Accept', 'application/json');
      this.http
        .get(`${this.apiUrl}comapnyMemberGet?memberId=${id}`, { headers })
        .subscribe(
          (editmemberData: any) => {
            this.editmemberData = editmemberData;
            this.memberedit = this.editmemberData?.data;
            let userids = id;
            this.designation.nativeElement.click();
            this.editmemberlist = this.editmemberlist.filter(
              (users: { id: string | null }) => users.id != userids
            );

            this.memberForm.controls['reportingTo'].setValue(
              this.memberedit.reportingTo
            );
  ...
  if (information.dob && !/^\d{4}-\d{2}-\d{2}$/.test(information.dob)) {
    const currentDate = new Date(information.dob); // Convert to Date object
    ...
    if (token) {
      ...
      const headers = new HttpHeaders()
        ...
        .set('Accept', 'application/json');
      this.http
        .post(`${this.apiUrl}comapnyMemberEdit`, formData, { headers })
        .subscribe(
          (response: any) => {
            if (response.status === true) {
              this.clearaddform.resetForm();
  ...
  editTeamGroup(id: number): void {
    ...
    this.http.get(`${this.apiUrl}teamGet?teamId=${id}`, { headers }).subscribe(
      (editteamgroupData: any) => {
        ...
        this.editmembers_id = (this.editteamgroups.members_id ?? []).map(
          (item: { id: any; name: any }) => ({
            item_id: item.id,
            item_text: item.name,
          })
  ...
  deleteTeamGroup(id: number) {
    ...
    if (token) {
      ...
      this.http.post(`${this.apiUrl}teamDelete`, teamId, { headers }).subscribe(
        (response: any) => {
          if (response.status === true) {
            const elementToClick = this.elementRef.nativeElement.querySelector(
              '.popup_close_btn_delete_team'
            );
            if (elementToClick) {
              elementToClick.click();
            }
  ...
  updateTeam(): void {
    ...
    if (token) {
      ...
      const membersIds = this.editTeamForm.value.members_id
        .map((item: any) => item.item_id)
        .join(', ');
      ...
      this.http.post(`${this.apiUrl}teamEdit`, payload, { headers }).subscribe(
        (response: any) => {
          if (response.status === true) {
            const elementToClick = this.elementRef.nativeElement.querySelector(
              '.popup_close_btn_team_edit'
            );
            if (elementToClick) {
              elementToClick.click();
            }
